...
[~,~,raw_once]= xlsread('CNIR_7T_HCP_survey_once.xlsx');
[~,~,raw_multiple] = xlsread('CNIR_7T_HCP_survey_multiple.xlsx');
Q = [];

idx.once = [54 74 96 120 130 140 size(raw_once,2)+1];
idx.once_names = {'STAI-Y2', 'K-RRS', 'IPI', 'ERQ', 'BFI', 'PWB'};
idx.multiple = [6 23 43 63 83 size(raw_multiple,2)+1];
idx.multiple_names = {'SMPQ', 'CES-D', 'PANAS', 'STAI-Y1', 'SL'};

%% once 1: STAI-Y2 (STAIYT)
que_num = 1;
clear raw_STAIY2
raw_STAIY2(:,1:idx.once(que_num+1)-idx.once(que_num)) = raw_once(:,idx.once(que_num):idx.once(que_num+1)-1);
...
que_num = 2;
clear raw_RRS
raw_RRS(:,1:idx.once(que_num+1)-idx.once(que_num)) = raw_once(:,idx.once(que_num):idx.once(que_num+1)-1);
...
que_num = 3;
clear raw_IPI
raw_IPI(:,1:idx.once(que_num+1)-idx.once(que_num)) = raw_once(:,idx.once(que_num):idx.once(que_num+1)-1);
...
que_num = 4;
clear raw_ERQ
raw_ERQ(:,1:idx.once(que_num+1)-idx.once(que_num)) = raw_once(:,idx.once(que_num):idx.once(que_num+1)-1);
...
que_num = 5;
clear raw_bfi
raw_bfi(:,1:idx.once(que_num+1)-idx.once(que_num)) = raw_once(:,idx.once(que_num):idx.once(que_num+1)-1);

for i=1:size(raw_bfi,1)
    for j=1:size(raw_bfi,2)
        if strfind(raw_bfi{i,j},'전혀 그렇지 않음')
            raw_bfi{i,j}=1;
        elseif strfind(raw_bfi{i,j},'그렇지 않은 편')
            raw_bfi{i,j}=2;
        elseif strfind(raw_bfi{i,j},'보통')
            raw_bfi{i,j}=3;
        elseif strfind(raw_bfi{i,j},'그런 편')
            raw_bfi{i,j}=4;
        elseif strfind(raw_bfi{i,j},'항상 그런 편')
            raw_bfi{i,j}=5;
        end
    end
end
...
% Sum
extraversion = 6 - cell2mat(raw_bfi(2:end,1)) + cell2mat(raw_bfi(2:end,6));
raw_bfi(2:end,end+1) = num2cell(extraversion);
raw_bfi{1,end} = 'extraversion';

agreeableness = 6 - cell2mat(raw_bfi(2:end,7)) + cell2mat(raw_bfi(2:end,2));
raw_bfi(2:end,end+1) = num2cell(agreeableness);
raw_bfi{1,end} = 'agreeableness';

conscientiousness = 6 - cell2mat(raw_bfi(2:end,3)) + cell2mat(raw_bfi(2:end,8));
raw_bfi(2:end,end+1) = num2cell(conscientiousness);
raw_bfi{1,end} = 'conscientiousness';

neuroticism = 6 - cell2mat(raw_bfi(2:end,4)) + cell2mat(raw_bfi(2:end,9));
raw_bfi(2:end,end+1) = num2cell(neuroticism);
raw_bfi{1,end} = 'neuroticism';

openness = 6 - cell2mat(raw_bfi(2:end,5)) + cell2mat(raw_bfi(2:end,10));
raw_bfi(2:end,end+1) = num2cell(openness);
raw_bfi{1,end} = 'openness';

% Fill structure
Q.BFI.total = raw_bfi;
Q.BFI.text(:,1) = raw_bfi(1,1:end-5)';
Q.BFI.text(1:5,2) = raw_bfi(1,end-4:end)';
Q.BFI.data = cell2mat(raw_bfi(2:end,1:end-5));
Q.BFI.result = cell2mat(raw_bfi(2:end,end-4:end));

%% once 6: PWB
que_num = 6;
clear raw_pwb
raw_pwb(:,1:idx.once(que_num+1)-idx.once(que_num)) = raw_once(:,idx.once(que_num):idx.once(que_num+1)-1);
...
que_num = 1;
clear raw_SFMPQ
raw_SFMPQ(:,1:idx.multiple(que_num+1)-idx.multiple(que_num)) = raw_multiple(:,idx.multiple(que_num):idx.multiple(que_num+1)-1);
...
que_num = 5;
clear raw_SL
raw_SL(:,1:idx.multiple(que_num+1)-idx.multiple(que_num)) = raw_multiple(:,idx.multiple(que_num):idx.multiple(que_num+1)-1);

for i = 1:size(raw_SL,1)
    for j = 1:size(raw_SL,2)
        if strfind(raw_SL{i,j},'전혀 동의하지 않는다(1)')
            raw_SL{i,j}=1;
        elseif strfind(raw_SL{i,j},'동의하지 않는다(2)')
            raw_SL{i,j}=2;
        elseif strfind(raw_SL{i,j},'약간 동의하지 않는다(3)')
            raw_SL{i,j}=3;
        elseif strfind(raw_SL{i,j},'보통이다(4)')
            raw_SL{i,j}=4;
        elseif strfind(raw_SL{i,j},'약간 동의하는 편이다(5)')
            raw_SL{i,j}=5;
        elseif strfind(raw_SL{i,j},'동의한다(6)')
            raw_SL{i,j}=6;
        elseif strfind(raw_SL{i,j},'매우 동의한다(7)')
            raw_SL{i,j}=7;
        end
    end
end

raw_SL(2:end, end+1) = num2cell(sum(cell2mat(raw_SL(2:end,:)),2));
raw_SL{1,end} = 'Satisfaction for life scale';

% Fill structure
Q.SL.total = raw_SL;
Q.SL.text(:,1) = raw_SL(1,1:end-1)';
Q.SL.text{2,1} = raw_SL{1,6};
Q.SL.data = cell2mat(raw_SL(2:end, 1:end-1));
Q.SL.result = cell2mat(raw_SL(2:end,end));
...
%% SAVE
savedir = pwd; 
savename = fullfile(savedir, '7T_questionnaire.mat');
% save(savename, 'Q');